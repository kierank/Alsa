...
//                        chip->pcm_granularity);
        ...
        printk(KERN_INFO "%s, set blocksize on board %d\n",
                        __func__,
                        snapped_gran);
...
//                        (substream->stream == SNDRV_PCM_STREAM_CAPTURE));
        ...
        case LX_IP:
        case LX_IP_MADI:
                err = snd_pcm_hw_constraint_list(runtime,
                                                 0,
                                                 SNDRV_PCM_HW_PARAM_RATE,
...
static void lx_trigger_stream_stop(struct lx_chip *chip, unsigned int is_capture)
{
        int err;
//        printk(KERN_DEBUG  "%s\n", __func__);
        ...
        if (err < 0) {
                printk(KERN_ERR LXP "couldn't stop pipe\n");
        }
        else {
                if (is_capture == 0) {
...
//        channels);
        ...
        if (substream->runtime->period_size < chip->pcm_granularity) {
                printk(KERN_ERR
                    "period size (%d) has to be multiple of " \
                    "dma granularity (%d)\n",
        ...
        (unsigned int)substream->runtime->period_size%chip->pcm_granularity);
                ...
                period_multiple_gran =
                        substream->runtime->period_size/chip->pcm_granularity;
                if (is_capture == 0) {
        ...
        if ((chip->lx_type == LX_IP) || (chip->lx_type == LX_IP_MADI)) {
                ...
                for (i = 0; i < loop; ++i) {
                        ...
                        if (reg_mbox3  == PLX_READY) {
                                printk(KERN_INFO "%s, xilinx reset done\n",
                                                __func__);
//                              printk(KERN_DEBUG
...
int lx_init_xilinx_test(struct lx_chip *chip)
{
        ...
        printk(KERN_INFO  "%s, Xilinx/MicroBlaze access test successful\n",
                        __func__);
...
//                                i);
 ...
 ethersound_initialized:
                printk(KERN_INFO  "%s, LX initialized\n", __func__);
        return 0;
}
...
int lx_init_get_version_features(struct lx_chip *chip)
{
        ...
        if (err == 0) {
                ...
                printk(KERN_INFO "%s, DSP version: V%02d.%02d #%d\n",
                           __func__,
                           (dsp_version>>16) & 0xff,
                           (dsp_version>>8) & 0xff,
                           dsp_version & 0xff);
...
//                        i);
                        ...
                        __func__,
                        chip->mac_address[0], chip->mac_address[1],
                        chip->mac_address[2], chip->mac_address[3],
                        chip->mac_address[4], chip->mac_address[5]);
...
int lx_pcm_create(struct lx_chip *chip)
{
        ...
        u32 size = MICROBLAZE_CHANNELS_MAX * /* channels */
                   3 *                       /* 24 bit samples */
