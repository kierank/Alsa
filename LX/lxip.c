...
static struct snd_pcm_hardware lx_ip_caps = {
	...
	.buffer_bytes_max = LXIP_USE_CHANNELS_MAX*LXIP_GRANULARITY_MAX*LXIP_PERIOD_MULTIPLE_GRAN_MAX*LXIP_SAMPLE_SIZE_MAX*LXIP_USE_PERIODS_MAX, //channels_max*period_bytes_max
	.period_bytes_min = LXIP_USE_CHANNELS_MIN*LXIP_GRANULARITY_MIN*LXIP_PERIOD_MULTIPLE_GRAN_MIN*LXIP_SAMPLE_SIZE_MIN,
	.period_bytes_max = LXIP_USE_CHANNELS_MAX*LXIP_GRANULARITY_MAX*LXIP_PERIOD_MULTIPLE_GRAN_MAX*LXIP_SAMPLE_SIZE_MAX,
...
//TODO SJR Clock Mode verrue pour le moteur audio Digigram.
...
const char *const lxip_granularity_names[] = {  "8", "16", "32", "64", "128", "256", "512"};
			       ...
			       struct snd_ctl_elem_value *value)
{
	...
	changed = lxip_granularity_value[value->value.enumerated.item[0]] != chip->pcm_granularity;
	if (changed) {
//		chip->pcm_granularity = lxip_granularity_value[value->value.enumerated.item[0]];
		//set_gran if allow
		printk(KERN_DEBUG "%s set gran to %d\n", __func__, chip->pcm_granularity);
		err = lx_set_granularity(chip, lxip_granularity_value[value->value.enumerated.item[0]]);
		if (err < 0) {
...
int lx_ip_pcm_create(struct lx6464es *chip)
{
	int err;
	struct snd_pcm *pcm;
...
//	printk(KERN_DEBUG  "%s\n", __func__);
	...
	if(chip->lx_type == LX_IP) {
		...
		err = snd_pcm_new(chip->card, (char *)"LX_IP_MADI", 0, 1, 1, &pcm);
	}
	if (err < 0)
		return err;
...
#error "kernel not supported"
	...
	chip->pci = pci;
	chip->irq = -1;
	if(pci->subsystem_device == PCIEX_SUBDEVICE_ID_DIGIGRAM_LXIP_SERIAL_SUBSYSTEM){
		chip->lx_type = LX_IP;


	}else if (pci->subsystem_device == PCIEX_SUBDEVICE_ID_DIGIGRAM_LXIP_MADI_SERIAL_SUBSYSTEM){
...
//	chip->port_plx_remapped = ioport_map(chip->port_plx, pci_resource_len(pci, 1));
