...
int lx_madi_pcm_create(struct lx_chip *chip)
{
	...
	size =	MADI_USE_CHANNELS_MAX * /* channels */
		...
		struct lx_chip **rchip)
{
	...
	static struct snd_device_ops ops = {.dev_free = snd_lx_dev_free, };
...
#error "kernel not supported"
	...
	chip->pci = pci;
	chip->irq = -1;
	if (pci->subsystem_device == PCI_SUBDEVICE_ID_DIGIGRAM_LXMADI_SUBSYSTEM)
...
/*	set default internal card conf to local*/
	...
	if (err < 0) {
		dev_err(&pci->dev, "%s, error during DSP initialization\n",
				__func__);
		goto device_new_failed;
	}
	...
	if (err < 0) {
		dev_err(&pci->dev, "%s,lx_madi_pcm_create failed\n", __func__);
		goto device_new_failed;
	}
