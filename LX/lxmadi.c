...
struct clocks_info {
        ...
        unsigned char clock_sync;      /* synchro source madi, word clock,
                                        * interne */
                ...
                if (lx_chips_slave != NULL) {
                        ...
                        for (i = 0; i< 10; i++) {
                                lx_madi_get_clocks_status(lx_chips_slave,
                                                          &clocks_informations);
                                if (clocks_informations.word_clock_freq ==
                                                        clock_frequency) {
                                        break;
                                }else{
                                        mdelay(10);
                        ...
                        if (clocks_informations.word_clock_freq !=
                                                        clock_frequency) {
                                printk(KERN_ERR
                                "%s, be careful Master and Slave looks not " \
                                "synchronize by wordclock\n", __func__);
                        }
                }
        }
...
const int const madi_internal_clock_frequency_value[] =
                                {  44100, 48000, 88200, 96000 };
                        ...
                        struct snd_ctl_elem_info *info)
{
        ...
        return snd_ctl_enum_info(info,
                                 1,
                                 4,
                                 madi_internal_clock_frequency_names);
}

static int snd_internal_clock_frequency_iobox_get(
                                        struct snd_kcontrol *kcontrol,
                                        struct snd_ctl_elem_value *value)
{
        struct lx_chip *chip = snd_kcontrol_chip(kcontrol);
...
static int snd_internal_clock_frequency_iobox_put(
                                        struct snd_kcontrol *kcontrol,
                                        struct snd_ctl_elem_value *value)
{
        struct lx_chip *chip = snd_kcontrol_chip(kcontrol);
                               ...
                               struct snd_ctl_elem_value *value)
{
        ...
        if (changed) {
                ...
                case LXMADI_SYNC_SLAVE :
                        ...
                        break;
                default :
                        printk(KERN_WARNING "unknown sync\n");
                        break;
                }
                ...
                snd_ctl_notify(chip->card,
                               ...
                               &chip->mixer_wordclock_out_ctl->id);
        }
        return changed;
}
const char *const madi_granularity_names[] =
                        {  "8", "16", "32", "64", "128", "256", "512"};
const int const madi_granularity_value[] =
                        {  8, 16, 32, 64, 128, 256, 512 };
...
//        printk(KERN_DEBUG  "%s %d\n", __func__, runtime->rate);
        ...
        case LXMADI_CLOCK_SYNC_MADI:
                ...
                if (runtime->rate != clocks_informations.madi_freq) {
                        printk(KERN_WARNING
                        "%s expected rate and madi clock are " \
                        "different expected %d found %d\n",
                        __func__,
                        runtime->rate,
                        clocks_informations.madi_freq);
                        err = -EINVAL;
                ...
                if (chip == lx_chips_master) {
                        if (lx_chips_slave != NULL) {
                                ...
                                clocks_informations.word_clock_freq !=
                                                runtime->rate) {
                                        printk(KERN_ERR
                                        "%s, be careful Master and " \
                                        "Slave looks not synchronize " \
                                        "by wordclock\n", __func__);
                                }
                        }
                }
        ...
        case LXMADI_CLOCK_SYNC_WORDCLOCK:
                ...
                if (runtime->rate !=
                                clocks_informations.word_clock_freq) {
                        printk(KERN_WARNING
                               "%s expected rate and word clock " \
                               "are different expected %d found %d\n",
                               __func__,
                               runtime->rate,
                               clocks_informations.word_clock_freq);
                        err = -EINVAL;
...
//                        runtime->rate = clocks_informations.internal_freq;
        ...
        default:
                //unknown sync....
                printk(KERN_WARNING "unknown clock sync\n");
                break;
        }
...
int lx_madi_pcm_create(struct lx_chip *chip)
{
        int err;
        struct snd_pcm *pcm;
...
#error "kernel not supported"
        ...
        if (pci->subsystem_device ==
                        PCIEX_SUBDEVICE_ID_DIGIGRAM_LXMADI_SERIAL_SUBSYSTEM) {
